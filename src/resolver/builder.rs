...
fn build_module_queries(
    ...
    module.structured_types = module
        .structured_types
        .into_iter()
        .map(|st| build_structured_type_queries(ctx, current_lang.clone(), st))
        .collect();
    module.free_functions = module
        .free_functions
        .into_iter()
        .map(|ff| build_function_queries(ctx, ff, None, false))
        .collect();
    module.impl_blocks = module
        .impl_blocks
        .into_iter()
        .map(|ib| build_impl_block_queries(ctx, current_lang.clone(), ib))
        .collect();
...
fn build_structured_type_queries(
    ctx: &BuilderContext,
    current_lang: Option<String>,
    ...
    st.super_types = st
        .super_types
        .into_iter()
        .map(|t| substitute_type(ctx, t, false))
        .collect();
    st.annotations = st
        .annotations
        .into_iter()
    ...
    st.methods = st
        ...
        .into_iter()
        .map(|m| {
            build_function_queries(
                ctx,
                m,
                Some(self_query.clone()),
                lang_config.implicit_first_param_as_self,
            )
        })
        .collect();
    st.nested_types = st
        .nested_types
        .into_iter()
        .map(|n| build_structured_type_queries(ctx, current_lang.clone(), n))
        .collect();
...
fn build_function_queries(
    ctx: &BuilderContext,
    mut ff: Function,
    self_query: Option<Query>,
    implicit_first_param_as_self: bool,
) -> Function {
    ctx.stack.borrow_mut().push_scope();
        ...
        .map(|mut p| {
            ...
            if let Some(name) = &p.name {
                if is_first && implicit_first_param_as_self {
                    ...
                    ctx.stack
                        .borrow_mut()
                        .define_symbol(name.clone(), q.clone());
                }
            }
            is_first = false;
            p
...
fn build_block_queries(ctx: &BuilderContext, mut block: Block) -> Block {
    ...
    block.calls = block
        .calls
        .into_iter()
        .map(|c| substitute_type(ctx, c, false))
        .collect();
    block.instantiates = block
        .instantiates
        .into_iter()
        .map(|i| substitute_type(ctx, i, false))
        .collect();
    block.accesses = block
        .accesses
        .into_iter()
        .map(|a| substitute_type(ctx, a, false))
        .collect();
    ...
    block.sub_blocks = block
        .sub_blocks
        .into_iter()
        .map(|b| build_block_queries(ctx, b))
        .collect();
...
fn substitute_type(ctx: &BuilderContext, tr: TypeRef, is_call: bool) -> TypeRef {
    match tr {
        TypeRef::Unresolved(qn) if !qn.is_empty() => {
            ...
            let mut base_query = if let Some(local_query) = ctx
                .stack
                .borrow()
                .frames
                .iter()
                .rev()
                .find_map(|f| f.symbols.get(first))
            {
                local_query.clone()
            } else {
