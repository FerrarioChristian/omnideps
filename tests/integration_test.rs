use std::fs;
use walkdir::WalkDir;
use language_agnostic_analyzer::extractor::{self, full_analysis};
use std::path::Path;

#[test]
fn test_benchmarks_analysis() {
    let benchmarks_dir = Path::new("benchmarks");
    assert!(benchmarks_dir.exists(), "La directory benchmarks non esiste!");
    
    let outputs_dir = Path::new("tests/outputs");
    if !outputs_dir.exists() {
        fs::create_dir_all(outputs_dir).expect("Impossibile creare la directory di output dei test");
    }

    for entry in WalkDir::new(benchmarks_dir).into_iter().filter_map(|e| e.ok()) {
        let path = entry.path();
        ...
        if path.is_file() {
            ...
            let lang = match ext {
                "rs" => extractor::languages::rust(),
                "java" => extractor::languages::java(),
                "py" => extractor::languages::python(),
                "c" | "h" => extractor::languages::c(),
                "cpp" | "cxx" | "cc" | "hxx" => extractor::languages::cpp(),
            ...
            assert!(result.is_ok(), "L'analisi ha fallito per il file {:?}", path);
