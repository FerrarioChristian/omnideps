...
dependencies, changing one module could break others in
...
immense resource consumption and fragmented, heavyweight, difficult
to maintain tooling.
...
languages through a single unified pipeline by transorming it to a
standardized Intermediate Representation, to generate a
the final \textbf{Dependency Graph}.
...
To ensure that the system is extensible, structurally robust, and
entirely independent of the syntactic peculiarities of individual
languages, the project adopts an approach based on
\textbf{algebraic data types} \cite{pierce_types_2002} and
...
subsequent phases to \textbf{operate} exclusively \textbf{on standardized
...
\begin{figure}[htpb]
  \centering
  \includegraphics[width=\textwidth]{images/introduction/analysis_pipeline.png}
  \caption{The functional pipeline of the analyzer.}
...
overall semantic consistency of the architecture. In practice,
adding support for a new language requires implementing only the
extraction heuristics for Phase 1, without modifying the resolution
engine, provided that the new language's semantic features (e.g.,
module strategies, self-parameter conventions, import semantics) are
already covered by the existing configuration parameters. If the new
language introduces novel semantic patterns not yet modeled by the
configuration, limited extensions to the resolution engine may be
necessary\cite{vacchi_neverlang_2015}.
...
\subsection{Source Code Availability and Reproducibility}
The \textit{Omnideps} framework developed during this thesis is
entirely open-source. For instructions on how to access the
repository, download the standalone executables, replicate the
benchmarks, or build the project from source, please refer to
Appendix \ref{app:reproducibility}.

\section{Organization of the Thesis}
\todo{Verify chapter structure matches final version}

The remainder of this thesis is organized as follows:

\begin{itemize}
  \item \textbf{Chapter \ref{ch:background}} reviews the foundational concepts
    of static analysis, code smells, dependency graphs, parsing
    technologies, name resolution, and type systems.
  \item \textbf{Chapter \ref{ch:ir_formalization}} formally defines the
    language-agnostic Intermediate Representation (IR) that serves as the
    central data model throughout the pipeline.
  \item \textbf{Chapter \ref{ch:analysis_pipeline}} presents an overview of
    the three-phase analysis pipeline, describing the input, output, and
    key properties of each phase.
  \item \textbf{Chapter \ref{ch:extraction}} details Phase 1 (Syntactic
    Extraction): the heuristic-based classification and parsing of
    Concrete Syntax Trees into the IR.
  \item \textbf{Chapter \ref{ch:name_resolution}} details Phase 2 (Name
    Resolution): the query algebra, Scope Tree construction, and lexical
    scope climbing algorithm.
  \item \textbf{Chapter \ref{ch:graph_construction}} details Phase 3 (Graph
    Construction): the flattening of the resolved IR into a dependency
    graph and its visualization.
  \item \textbf{Chapter \ref{ch:evaluation_results}} presents the
    evaluation of the
    tool across multiple benchmark projects and programming languages.
  \item \textbf{Chapter \ref{ch:conclusions}} draws conclusions and outlines
    potential directions for future work.
\end{itemize}

Additionally, two appendices are provided to assist the reader and
ensure reproducibility:
\begin{itemize}
  \item \textbf{Appendix \ref{apx:notation}} provides a comprehensive
    glossary of the mathematical symbols and notation used to
    formalize the Intermediate Representation and the algebraic query engine.
  \item \textbf{Appendix \ref{app:reproducibility}} details the
    open-source availability of the \textit{Omnideps} framework. It
    provides the repository link and exhaustive instructions for
    end-users and developers to download the executables, build the
    project from source, and replicate the experimental benchmarks.
\end{itemize}
