...
\begin{itemize}
    \item \textbf{Tuple (Prodotto Cartesiano)} $\langle x_1, x_2, \dots, x_n \rangle$: Rappresentano tipi composti (corrispondenti alle \texttt{struct}  nel codice sorgente). Ogni elemento della tupla definisce una proprietà essenziale dell'entità.
    \item \textbf{Unione Disgiunta (Tipo Somma)} $A \mid B \mid C$: Indica che un valore può appartenere a uno e un solo tipo tra quelli elencati (corrispondente agli \texttt{enum} nel codice). È utilizzato per modellare il polimorfismo o l'incertezza.
    ...
    \item \textbf{Opzionalità} $X^?$: Indica un valore che può essere presente (di tipo $X$) oppure assente (corrispondente a \texttt{Option<T>}).
...
\begin{itemize}
    ...
    \item $\tau$ (\textbf{Type Reference}): Un riferimento astratto a un tipo. Rappresentato da un tipo somma a quattro stati che ne modella il ciclo di vita durante l'analisi: \texttt{Prim} (primitivo di base), \texttt{Unresolved} (nome grezzo appena estratto), \texttt{Resolved} (percorso assoluto accertato), o \texttt{Failed} (tentativo di risoluzione non andato a buon fine).
\end{itemize}
...
\begin{itemize}
    ...
    \item $\sigma$ (\textbf{Signature}): La firma architetturale di una funzione, composta dalla tupla $\langle \vec{P}, \tau_{ret} \rangle$. Astrae i tipi di input e output isolandoli dal componente implementativo.
    \item $\mathcal{F}$ (\textbf{Function}): Un componente funzionale universale definito dalla coppia $\langle \mathcal{I}, \sigma \rangle$. Il suo ruolo architetturale (metodo o funzione libera) è determinato unicamente dal costrutto in cui è annidato.
\end{itemize}

\subsection{Entità Architetturali}
\begin{itemize}
    \item $\mathcal{S}$ (\textbf{Structured Type}): L'astrazione di classi, struct, interfacce e trait. Aggrega campi, funzioni, gerarchie di ereditarietà e tipi annidati.
    \item $\mathcal{B}_{lk}$ (\textbf{Implementation Block}): Costrutto sintattico di estrazione temporaneo tipico di linguaggi come Rust o C++. Associa un insieme di funzioni a un tipo target. Viene appiattito all'interno di $\mathcal{S}$ durante la Fase di Risoluzione, scomparendo dal modello finale.
    \item $\mathcal{C}$ (\textbf{Component}): Il tipo somma universale che abbraccia le entità analizzabili definitive del sistema (Moduli, Tipi Strutturati, Funzioni).
...
\begin{itemize}
    ...
    \item $G$ (\textbf{Graphing}): L'ultima fase che converte il modello a oggetti risolto nel grafo delle dipendenze $\mathcal{G}$.
\end{itemize}
